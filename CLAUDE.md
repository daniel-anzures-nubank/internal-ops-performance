...
**Manual adjustments** (per-agent / per-date carve-outs, training & shadowing windows, maternity leave, outage dates, cross-support queue exclusions, etc.) are sourced from this spreadsheet, which is the source of truth for adjustments: [https://docs.google.com/spreadsheets/d/1Y5P6LijLxT6hFTd69DiSPBTUPKHO-m_6zzrs-PmOjfU/edit?gid=720896495#gid=720896495](https://docs.google.com/spreadsheets/d/1Y5P6LijLxT6hFTd69DiSPBTUPKHO-m_6zzrs-PmOjfU/edit?gid=720896495#gid=720896495). These are intentionally **not** applied in the raw data layer — they are the job of the **Adjustments layer** (`adjustments/`), which sits on top of the metric layer. See [Adjustments Layer](#adjustments-layer-adjustments) and `docs/adjustments_docs/README.md`.
...
- The top-level Python project — a **refactor in progress**, rebuilding the legacy pipeline as small, testable, PySpark-based code. Logic modules are pure `pyspark.sql` (DataFrame-in / DataFrame-out), unit-tested locally against a local SparkSession, and run **on Databricks** via a git-sourced orchestration job — see [Databricks Deployment](#databricks-deployment-job--git-folder--a-duality).

## Repository Layout

Flat layout. No `src/` wrapper, no package directories, no `__init__.py` files — with one exception: `adjustments/__init__.py` must stay, because `adjustments` is imported as a package (`from adjustments.manual import ...`).
...
- `metrics_data/` — Pure-PySpark logic modules that build the **raw data tables** (`adherent_time`, `occupancy_time`, `jobs_raw`, `jobs_within_sla`, `quality_evaluations`, `shrinkage_slots`, `tnps_responses`, `wows`, `content_csat`). One module per raw table; minimal filtering only (business exclusions are deferred to the metrics layer). See [Raw Data Tables](#raw-data-tables-metrics_data).
...
- `metrics/` — Pure-PySpark logic modules that build the **finished metric tables** from the `io_*_raw` tables (one module per metric — see [Metric Tables](#metric-tables-metrics) for the full catalog); shared bucketing/aggregation lives in `metrics/metric_utils.py`. This is where business exclusions, benchmarks, and ratios that the raw layer deferred get applied.
...
- `adjustments/` — Pure-PySpark logic modules for the **manual adjustments layer** — the per-agent / per-date carve-outs the raw and metric layers defer (cross-support exclusions, leave reclassifications, training/shadowing windows, outage dates, DIME-squad exclusions, per-agent Index carve-outs). **Implemented**: `adjustments/manual.py` holds the apply-helpers (e.g. `drop_slot_windows`, `drop_cross_support_jobs`, `reclassify_dime_slots`) consumed by the metric modules and build scripts; the sheet download/sync lives in `scripts/adjustments_scripts/`. Source of truth is the adjustments Google Sheet. See [Adjustments Layer](#adjustments-layer-adjustments).
...
- `scripts/` — Runnable entry points. Thin orchestrators over the **shared `db.py` Spark transport** (on Databricks each script resolves the cluster's ambient SparkSession via `db.open_connection`), each with its own CLI. Organized into subfolders: `scripts/metrics_data_scripts/` holds one `build_*.py` per raw table (imports the matching `metrics_data/` module and writes the result to Delta); `scripts/metrics_scripts/` holds one `build_<metric>.py` per metric (reads the `io_*_raw` table(s) via `db.read_table`, imports the matching `metrics/` module, writes the `io_*_metric` table); `scripts/adjustments_scripts/` holds the adjustments-sheet download/sync (`download_adjustments.py`, `sync_adjustments.py`); `scripts/adhoc/` holds dated one-off deliverable scripts (not pipeline tasks). The data-quality runner `check_extractor_data_quality.py` stays at the `scripts/` root.
- `tests/` — PySpark `test_*.py` unit tests that build synthetic Spark DataFrames against a shared **local SparkSession** (`tests/conftest.py`) — no warehouse — organized to mirror the code: `tests/metrics_data/` (raw-table modules), `tests/metrics/` (metric modules), `tests/extractors/` (reserved for extractor checks). The DQ engine `checks.py` (its check primitives take pandas DataFrames) + `test_checks.py` and `tests/parity/` live at the `tests/` root. Imports resolve via `pythonpath = [".", "tests", "metrics_data", "metrics"]` in `pyproject.toml`.
...
| `io_ntpj_metric`                 | `ntpj` — Core/Fraud: actual / expected job time (≤ 100%, duration, lower-better). **Content: SLA-weighted compliance (≥ target %, higher-better, ≤100)** — a different metric under the same name (see NTPJ note below)                     | `io_jobs_raw` (Core/Fraud) + `io_jobs_within_sla_raw` (Content) | one row per agent per day/week/month/quarter/semester/year                       |
...
| `io_ntpj_xforce_metric`          | `ntpj_xforce` = share of an XForce's agents on-target — **team-aware**: Core/Fraud `ntpj ≤ 100`, Content `ntpj ≥ 95` (SLA). Roll-up of `io_ntpj_metric` (legacy `ntpj_xforces`)                     | `io_ntpj_metric`                                                                                                                        | **XForce roll-up**, week + month only                                            |
...
**Xpeer Index is a composite** (legacy `index_agent`): it reads the other `io_*_metric` tables (not an `io_*_raw` table) and folds them into a single mean per `(agent, date_reference, date_granularity)`. Adherence is the **driver** (an agent is in the Index iff it has an Adherence row; `team`/dims come from there). Components are folded first — NTPJ around 100 (`≤100→100`, `100–200→200−x`, else 0), NO truncated at 100, WoWs `≥5→100` else `x/5*100`; tNPS / Quality / CSAT enter raw. The roster is **team- and era-dependent**: Core/Fraud = Adherence + NTPJ (+Quality from Feb, +NO from March); Content = Adherence + NTPJ (+NO and +CSAT from March); Social Media = Adherence + WoWs (+tNPS when present, +Quality from Feb, +NO from March). Quality/CSAT/tNPS drop from both sum and divisor when missing. Multi-month buckets anchor the era on the period's **end** month; pre-2026 buckets are dropped. Per-agent legacy carve-outs (e.g. the `nitza.zarza` NO suppression) are applied as documented hardcodes in `metrics/xpeer_index.py`, pending migration to the adjustments sheet.

Adherence drops non-productive slots (`activity_type_required` in `lunch_break` / `time_off` / `shrinkage`) before the ratio. The legacy `dimensioned_activity` **meeting/leave exclusion** and the **DIME-squad exclusion** (`agent_dime_squad` not in `wfm` / `credit_evolution` / `dote`, NULL dropped) are both applied as **fixed DIME filters in the raw layer** (`metrics_data/adherent_time.py` → `filter_dime`); the per-agent/outage adjustments come from the adjustments sheet and are applied in the metric layer (`adjustments/manual.py` → `drop_slot_windows` / `reclassify_dime_slots`).

NTPJ keeps finished jobs only; the cohort-wide monthly benchmark `exp_duration_job` (`SUM(duration)/SUM(count)` per `job_id`) uses a trailing `[M-4 … M]` window for months ≤ 2026-03 and the current month for months ≥ 2026-04, so the build script reads a 4-month look-back before `period_start`. Each agent's contribution is restricted to `required_activity_on_day_flag == 1` (the benchmark is not). Social Media has no NTPJ (no shuffle/OOS jobs). Cross-support queue exclusions, outage dates, and job/slot exclusions are applied in the metric layer from the adjustments sheet (`adjustments/manual.py` → `drop_cross_support_jobs` / `drop_excluded_jobs` / `drop_slot_windows`), plus the documented per-agent legacy hardcodes in `metrics/ntpj.py` (`HARDCODED_AGENT_DATE_EXCLUSIONS`, pending sheet migration) — see `docs/metrics_docs/ntpj.md`. The legacy Content "always 4-month window" benchmark rule is moot now that Content NTPJ is the SLA-based metric (below).

**Content NTPJ is a different metric (SLA-weighted compliance, not duration).** Legacy Content `ntpj_agent` = `ntpj_sla_old`: per Content OOS job (grouped by `content_id`, or per row for `macros`/`faq`/`ar`), a job earns its full SLA seconds if delivered within its OLD-SLA threshold else 0; the metric is `SUM(sla_met_seconds)/SUM(sla_seconds)*100` — higher-is-better, bounded ≤100. The SLA map lives in the **`Content - SLAs`** sheet tab (`adj_content_slas`), the per-job substrate is `io_jobs_within_sla_raw` (`metrics_data/jobs_within_sla.py`), and `build_ntpj.py` computes Core/Fraud from `io_jobs_raw` then **drops the Content output rows and unions** the SLA-based Content NTPJ (`metrics/content_sla_ntpj.py`) so `io_ntpj_metric` stays one `metric='ntpj'` table. Consumers are Content-aware: `xpeer_index` adds Content NTPJ **raw** (Core/Fraud fold around 100); `ntpj_xforce` uses `≥ 95` for Content.

Normalized Occupancy drops the same non-productive slots as adherence, then computes per-agent `occupancy = SUM(occupancy_minutes)/SUM(required_minutes)` and a monthly **district+shift benchmark** (legacy two-step: per-`(month, district, shift, squad)` occupancy ratio, then the mean of those squad ratios per `(month, district, shift)`). `numerator` carries the agent occupancy %, `denominator` the benchmark %, so `metric_value` = NO %. All four teams; the DIME-squad exclusion is a fixed raw-layer filter (`metrics_data/occupancy_time.py`), and per-agent/outage carve-outs are applied in the metric layer from the adjustments sheet (`drop_slot_windows` / `reclassify_dime_slots`), plus the documented `nitza.zarza` suppression hardcode.
...
Shrinkage takes `numerator = SUM(shrinkage_flag)` (the raw layer's pre/post-2026-03-01 slot rule) over `denominator = SUM(required_slot)`. The required-slot rule (applied here): drop `lunch_break` always, then a slot is required unless its `activity_type_required` is `dime_invalid_notation` (pre-cutover `< 2026-03-01`) or `time_off` (post-cutover `>= 2026-03-01`). All teams. Per-agent maternity/vacation reclassifications, training/shadowing exclusions, and outage dates are applied in the metric layer from the adjustments sheet (`adjustments/manual.py` → `drop_slot_windows` / `reclassify_dime_slots` / `apply_no_shrinkage`); the shrinkage-specific DIME-squad exclusion is a fixed raw-layer filter (`metrics_data/shrinkage_slots.py`). The raw also carries `controllable`/`uncontrollable` flags for a future breakdown.
...
**Status: partially implemented.** `adjustments/manual.py` implements the apply-helpers — `reclassify_dime_slots`, `drop_slot_windows`, `apply_no_shrinkage`, `drop_cross_support_jobs`, `drop_excluded_jobs`, `append_missing_dime_slots`, plus `read_adjustment_table` to read the synced `adj_*` Delta tables — and `scripts/adjustments_scripts/` (`download_adjustments.py` / `sync_adjustments.py`) syncs the sheet tabs to those tables. The helpers are applied in the **metric layer** (`metrics/adherence.py`, `metrics/normalized_occupancy.py`, `metrics/ntpj.py`, `metrics/shrinkage.py`, `metrics/improved_benchmarks.py`) and wired up by the corresponding build scripts; tests live in `tests/test_manual_adjustments.py` and `tests/test_download_adjustments.py`. **Remaining un-ported legacy hardcodes** deliberately stay in the metric modules pending migration to the sheet: `HARDCODED_AGENT_DATE_EXCLUSIONS` in `metrics/ntpj.py`, and the `nitza.zarza` NO-suppression constants in `metrics/normalized_occupancy.py` and `metrics/xpeer_index.py`. Cutover-date constants are intentionally per-module. When adding a new adjustment, mirror the layers: helper in `adjustments/`, sync/build wiring in `scripts/adjustments_scripts/`, tests, and one `docs/adjustments_docs/<name>.md` per adjustment. See `docs/adjustments_docs/README.md` for the catalog.
...
- **PySpark logic, transport isolated.** The logic modules (`metrics_data/`, `metrics/`, `adjustments/`) are pure `pyspark.sql` — DataFrame-in / DataFrame-out, no table IO, no env vars, no `dbutils`. Tests run them against a local SparkSession; on Databricks the same code runs unchanged on the cluster's ambient session. Session/catalog concerns live only in `db.py` and the `scripts/` entry points.
- **Flat layout.** No `src/`, no nested packages, no `__init__.py` unless something genuinely needs to be a Python package (today only `adjustments/` does — it is imported as `adjustments.manual`). Earn each new folder by feeling real pain — don't scaffold empty directories.
- **Transport in `scripts/` + `db.py`, logic in the logic modules.** Anything that opens a Spark session, reads env vars, parses CLI args, or touches tables lives in `scripts/` (via the shared `db.py` transport). Anything that takes a DataFrame and returns a DataFrame lives in `metrics_data/`, `metrics/`, or `adjustments/`. `tests/` holds the unit tests plus the DQ check primitives (`tests/checks.py`). This split is what makes `pytest` fast and the codebase portable.
...
- **Ground metric behavior in the legacy SQL — don't guess.** The `legacy/` notebooks are the **source of truth** for how every metric and table is computed until it's migrated. When porting a metric, diagnosing a parity gap, or explaining a divergence, open the corresponding legacy `.sql` and read the actual view / benchmark / filter definition instead of inferring it from the new code, the output table's shape, or a probe. Map notebook → deck via `legacy/CLAUDE.md`: **Core/Fraud** metrics come from the per-dataset notebooks (`[IO] Adherence Dataset.sql`, `[IO] Normalized Occupancy Dataset.sql`, `[IO] NTPJ Dataset.sql`, `[IO] Quality Dataset.sql`, `[IO] Shrinkage Dataset.sql`) orchestrated by `[IO] Performance 2026.sql`; **Social Media** and **Content** are self-contained in `[IO] Performance 2026 - Social Media Temp Fix.sql` / `[IO] Performance 2026 - Content Temp Fix.sql` (use the **Temp Fix** variant — it's the runnable SOT that materializes the `sm_temp_*` / `cont_temp_*` tables). The same metric can be defined *differently per deck* — e.g. the Content Normalized Occupancy benchmark is keyed on `squad_district` only, while the main deck uses `district + shift` — so always read the deck that owns the rows you're checking, not just "a" legacy file.
...
- `**tests/test_*.py**` — Build synthetic Spark DataFrames with the shared `spark` fixture (`tests/conftest.py`), exercise one module, assert on collected rows (the `checks.py` tests still build pandas frames and assert on the `CheckResult`). No network, no Databricks — a local SparkSession. Run with `.venv/bin/python -m pytest` (export `PYSPARK_PYTHON` to the same interpreter first).
- `**scripts/*.py**` — Thin orchestrators. argparse + the shared `db.py` Spark transport + a call into the matching logic module (`metrics_data/` / `metrics/`) or the DQ primitives (`tests/checks.py`). No business logic.
- `**db.py**` — The single warehouse transport (the only file that talks to Spark/Delta for table IO; `get_spark()` resolves the ambient SparkSession). Build scripts write via `publish()` (current table + `_snapshots` history + `pipeline_runs` registry; see [Run Snapshots & Registry](#run-snapshots--registry)), not `write_dataframe` directly.
