...
- **XForce Index** (legacy `index_xforce`): mean of up to four 0-100 components — shrinkage (target 23 for May/June-2026 **month** buckets, else 20: `≤ target → 100`, else `(100+target) − shrinkage`), xpeers_in_target (**rescaled**: `≥ 70 → 90 + (x−70)/3`, `< 70 → raw` — the re-export rule, applied to all history), average_xpeer_index (raw), improved_benchmark (`≥60→100`, else `improved/0.6`). NULL components → 0. The improved component joins **only where a row exists** (Core/Fraud month pre-cutover), so SM/Content and non-month grains stay 3-component. `numerator` = Σ components, `denominator` = 100×N.
