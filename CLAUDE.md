...
- **SM quality Sprinklr floor** — the Sprinklr SM feed is floored at `2026-05-01` (legacy `sm.report_date >= "2026-05-01"`). It **unions** with Playvox (which has no May cutoff — SM Playvox evals flow until they naturally end after 2026-05-15), exactly like legacy's SM-deck `qa_base`. There is no source switch and no quality date drops.
...
| `io_xpeers_in_target_metric`     | `xpeers_in_target` (XForce) + `xpeers_in_target_xplead` (+ SM/Content `_squad`/`_district` variants) = targets achieved / total targets (≥ 70%), Core/Fraud/SM/Content     | the agent-level `io_*_metric` tables + `io_content_csat_metric` (**composite**)                                                          | XForce + XPLead roll-ups, all six granularities                                  |
...
- **Quality** unions Playvox (all teams, no upper date bound) + Sprinklr SM (floored `>= 2026-05-01`), tagged by `source` — matching legacy's SM-deck `qa_base`; keeps the latest record per `(source, evaluation_id)`; averages `qa_score` (`scale = 1`). Content excluded (CSAT is its quality). **No quality dates are dropped** — the 2026-06-30 legacy re-export re-included the 03-27/04-09 rows (published `quality_io` / `sm_temp_quality` carry them).
...
- **Xpeers In Target** flags each agent in/out of target per component (adherence ≥95, ntpj ≤100 [Content ≥95 — its SLA NTPJ is higher-better; legacy quirk: the Content XPLead grain flags ≥100 pre-cutover], NO ≥100, quality ≥95, content_csat ≥95 [Content's Quality], tnps ≥88, wows ≥5) and reports `Σ achieved / Σ targets`. Two grains in one table (`xpeers_in_target` per XForce; `_xplead` per XPLead, `xforce` NULL) plus SM/Content `_squad`/`_district` variants (SM sums counts; Content averages the grain rows' metric_value with a constant-0 denominator — each matches its own legacy notebook). Content added per the 2026-06-30 legacy re-export (Content rows from Feb 2026 only, NTPJ/NOcc from March, XPLead month-only pre-cutover; the legacy 2025-12-01 stray rows are an upstream artifact, not reproduced). Era-gated like the Index; adherence is the driver; `team` is NULL by design. **The on-target thresholds were NOT changed by the 2026-06-30 re-export** — the 90-100 rescale lives only in `xforce_index`'s component fold.
