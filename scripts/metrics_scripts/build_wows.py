"""Build the WoWs metric and (optionally) write it to Databricks.
...
``metric_value`` is the number of WoWs; ``denominator`` carries the monthly
target (5) for reference.

Tables
------
* Input:  ``usr.danielanzures.io_wows_raw`` (override `--source`).
* Output: ``usr.danielanzures.io_wows_metric`` (override `--target`).
...
DEFAULT_SOURCE = "usr.danielanzures.io_wows_raw"
DEFAULT_TARGET = "usr.danielanzures.io_wows_metric"
