...
``tests/metrics/test_average_xforce_index.py``. Here we only:
  ...
  2. Read the XForce Index for the period (via `db.read_table`, scoped on
     ``date_reference``).
  3. Call ``compute_average_xforce_index`` to average to the XPLead level.
...
::
    ...
    uv run python scripts/metrics_scripts/build_average_xforce_index.py \\
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"XForce Index table (default: {DEFAULT_SOURCE}).",
    )
    parser.add_argument(
        "--target",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        expected_cols = [c for c, _ in IO_AVERAGE_XFORCE_INDEX_METRIC_SCHEMA]
        ...
        if args.dry_run:
            ...
            for g in GRANULARITIES:
                ...
                print(f"{g:>9}: {len(sub):,} rows, {sub['xplead'].nunique():,} xpleads")
