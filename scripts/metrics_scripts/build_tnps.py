...
and is covered by ``tests/metrics/test_tnps.py``. Here we only:
  ...
  3. Call ``compute_tnps`` to get the metric rows at all granularities
     (validity window, classify-then-COUNT(DISTINCT case_number), the
     pre-cutover 2026-03-27 outage-date drop).
...
hardcoded, cutover-gated constant applied inside ``compute_tnps`` (legacy SM
tNPS artifact, date < 2026-07-01).

Usage
-----
Runs on a Databricks cluster (``spark-submit`` / a Databricks job task)::

    python scripts/metrics_scripts/build_tnps.py \\
...
def main(argv: list[str] | None = None) -> int:
    ...
    with _log_step(f"read {args.source}"):
        responses = read_table(spark, args.source, args.period_start, args.period_end)
    ...
    if args.dry_run:
        ...
        if stats["mv_min"] is not None:
            print(
                f"metric_value (NPS %): min={stats['mv_min']:.1f}  "
