...
``tests/test_shrinkage_slots.py``. Here we only:
  ...
  3. Call ``compute_shrinkage_slots`` to get one row per DIME slot with the
...
::
    ...
    uv run python scripts/metrics_data_scripts/build_shrinkage_slots.py \\
...
from shrinkage_slots import IO_SHRINKAGE_SLOTS_SCHEMA, compute_shrinkage_slots  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with _log_step("compute_shrinkage_slots"):
            ...
            LOGGER.info("  %s rows in final shrinkage_slots frame", f"{len(result):,}")

        # Reorder to match the declared schema before writing.
        expected_cols = [c for c, _ in IO_SHRINKAGE_SLOTS_SCHEMA]
        result = result[expected_cols]

        if args.dry_run:
            LOGGER.info("Dry run — not writing. Summary:")
            print()
            print(f"Rows (slots):     {len(result):,}")
            print(f"Agents:           {result['agent'].nunique():,}")
            print(f"Dates:            {result['date'].nunique():,}")
            print(
                f"Squads (roster):  {result['squad'].nunique():,} "
                f"({', '.join(sorted(result['squad'].dropna().unique()))})"
            )
            print(
                f"Activity types:   {sorted(result['activity_type_required'].dropna().unique())}"
            )
            total_slots = len(result)
            total_shrink = int(result["shrinkage_flag"].sum())
            total_ctrl = int(result["controllable_shrinkage_flag"].sum())
            total_unctrl = int(result["uncontrollable_shrinkage_flag"].sum())
            print(
                f"shrinkage_flag:   {total_shrink:,} / {total_slots:,} slots "
                f"(controllable={total_ctrl:,}, uncontrollable={total_unctrl:,})"
            )
            print()
            print("Head:")
