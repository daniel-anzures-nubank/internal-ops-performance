"""Build the adherent_time raw dataset and (optionally) write it to Databricks.
...
``tests/metrics_data/test_adherent_time.py``. Here we only:
  ...
  3. Call ``compute_adherent_time`` to get the per-(agent, date, slot) result.
  4. Either print a summary (`--dry-run`) or replace the target Delta table.

Target table
------------
Default: ``usr.danielanzures.io_adherent_time_raw``. Override with ``--target``.
...
# Sibling top-level modules: `db.py` at the root, `adherence.py` in `metrics/`.
...
DEFAULT_TARGET = "usr.danielanzures.io_adherent_time_raw"
