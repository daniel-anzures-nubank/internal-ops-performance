...
Adherence drops non-productive slots (`activity_type_required` in `lunch_break` / `time_off` / `shrinkage`) before the ratio; the legacy `dimensioned_activity` meeting/leave carve-outs, DIME-squad exclusions, and per-agent/outage adjustments are deferred to a future Adjustments layer (the raw table doesn't carry `dimensioned_activity`).
