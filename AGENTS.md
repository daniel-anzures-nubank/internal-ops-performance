...
Adherence drops non-productive slots (`activity_type_required` in `lunch_break` / `time_off` / `shrinkage`) before the ratio. The legacy `dimensioned_activity` **meeting/leave exclusion is applied as a fixed DIME filter in the raw layer** (`metrics_data/adherent_time.py` → `filter_dime`); DIME-squad exclusions and per-agent/outage adjustments remain deferred to the Adjustments layer.
